# -*- coding: utf-8 -*-
"""python-sklearn-assignment.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/11P2UYXvRhJ5uOMGjxYIO-kDu5xod7lUk
"""
...
jovian.set_colab_id('11P2UYXvRhJ5uOMGjxYIO-kDu5xod7lUk')
...
Based on the initial look at the dataset, it is clearly visible that we have immense amount of paramters which can impact the prices of the houses. To have some high-level clue on which paramters/features will impact the pricing, let us have a few visualizations based on the following columns - 
- **MSZoning** - Identified the general zoning classification of the sale (Agriculture, Commercial, Floating Village Residential, Industrial, Residential High Density, Residential Low Density, Residential Low Density Park, Residential Medium Density)
- **LotArea** - Lot size in square feet.  
- **LotShape** - General shape of the property - Regular, slightly regular, moderately regular, irregular
- **Neighborhood** - Physical locations within Ames city limits
- **Housestyle** - Style of dwelling - one story, one-half story, two story, two-half story, split level, etc. 
- **Street** -  Type of road access to property
- **OverallQuality** - Rating of overall material and finish of the house
...
"""Let us now plot a boxplot to understand the value ranges - max, min, median for all the zones available in our data set.



"""

sns.boxplot(x = 'MSZoning', y = 'SalePrice', data = prices_df)
...
The graph given above indicates the following - 
- RL(Residential Low Density) has the maximum range in terms of salesprice and it starts somewhere around 30000 to over 700000 with median being somewhere around 200000. With RL we have a chances to have salesprice in a huge range. Being a Low density residential could be the reason why people are ready to pay more 
- C(commercial) zone seams to have least price median of all which indicates that salesprice in commercial place is lesser compared to other zones. Commercial place being crowded and noisy, people might be paying lesser for the house there 
- FV tends to have a larger median in comparison to all

Let us know try to plot a scatterplot to understand how LotArea is behaving against the Sale Price.
"""

sns.scatterplot(x = "LotArea", y = "SalePrice", hue = "MSZoning", data = prices_df)

"""The above graph indicates the following -
- Most number of values (almost more than 95%) seems to be lying under 50000 LotArea, which means that we have more data for LotArea around 50000 
- The price has a huge clustering around 100000 to 300000 and it shows that we have more number for records for this range of sale price 
- RL Zone tends to have lower area and sale price compared to all
"""

sns.scatterplot(x = "LotArea", y = "SalePrice", hue = "LotShape", data = prices_df)

#sns.violinplot(data = prices_df, x="LotArea", y="SalePrice", hue="LotShape", inner="quart", linewidth=1)
sns.violinplot(data = prices_df, x="Utilities", y="SalePrice", hue="LotShape")
...
#Drop the column Id and SalePrice as Id is not needed and SalePrice is to be predicted
#prices_df.drop(['Id', 'SalePrice'], axis = 1, inplace = True)
input_column_df_set = prices_df.drop(['Id', 'SalePrice'], axis = 1)
input_cols = input_column_df_set.columns
input_cols
